...
class Plot:
    def __init__(self):
        ...
        self.INSTANCE_PLOTS_DIR = self.ROOT_DIR / 'stdout' / 'instance-plots'
        self.RUN_DIR = self.ROOT_DIR / 'stdout' / 'runs'
        self.ANALYSIS_DIR = self.ROOT_DIR / 'stdout' / 'analysis'

        self.percentErrorVsNFitPower = 'log-linear'
        self.computationTimeVsNFitPower = 'log-linear'
        self.showAggConvergenceCurves = True
        self.showAggConvergenceStats = True

    def plotEuc2DInstances(self):
        if not self.INSTANCE_PLOTS_DIR.is_dir():
            print(f'WARNING: Cannot locate {self.INSTANCE_PLOTS_DIR}')
            os.makedirs(self.INSTANCE_PLOTS_DIR, exist_ok=True)
            return
        if not self.INSTANCE_DIR.is_dir():
            print(f'ERROR: Cannot locate {self.INSTANCE_DIR}')
            return
            ...
            if instanceFile.is_file() and (instanceFile.suffix == '.tsp'):
                try:
                    ...
                    if instance.edge_weight_type == 'EUC_2D':
                        ...
                        plt.savefig(rf'{self.INSTANCE_PLOTS_DIR}\{name}.png')
                        plt.close()
                    ...
                    if not self.RUN_DIR.is_dir():
                        print(f'Cannot locate {self.RUN_DIR}')
                        return
    ...
    def _thinXTicks(self, ax, positions, sortedN, maxLabels=20):
        # Draw a tick for every box, but only label every stride-th one so the
        # labels stay legible as the number of instances grows. We don't force
        # a label on the last n: it would sit right next to the final strided
        # label and overlap it.
        ax.set_xticks(positions)
        stride = max(1, -(-len(positions) // maxLabels))  # ceil division
        labels = [
    ...
    def plotComputationTimeVsN(self):
        if not self.ANALYSIS_DIR.is_dir():
            print(f'WARNING: Cannot locate {self.ANALYSIS_DIR}, creating it...')
            os.makedirs(self.ANALYSIS_DIR, exist_ok=True)
        if not self.RUN_DIR.is_dir():
            print(f'ERROR: Cannot locate {self.RUN_DIR}')
            return

        dataByN = {}
        for instanceDir in self.RUN_DIR.iterdir():
        ...
        _blue = (0.122, 0.467, 0.706)
        _blue_fill = (0.122, 0.467, 0.706, 0.2)
        ...
        for patch in bp['boxes']:
            patch.set_facecolor(_blue_fill)
            patch.set_edgecolor(_blue)
            patch.set_linewidth(1.5)
        for whisker in bp['whiskers']:
            whisker.set_color(_blue)
            whisker.set_linewidth(1.5)
        for cap in bp['caps']:
            cap.set_color(_blue)
            cap.set_linewidth(1.5)
        for flier in bp['fliers']:
            flier.set(marker='o', color=_blue, alpha=0.5, markersize=4)
        ...
        self._drawFitCurve(ax, nArr, posArr, medians, self.computationTimeVsNFitPower, var='t')
        ...
        plt.tight_layout()
        plt.savefig(rf'{self.ANALYSIS_DIR}\computation_time_vs_n.png')
        plt.close()
    ...
    def plotPercentErrorVsN(self):
        if not self.ANALYSIS_DIR.is_dir():
            ...
            os.makedirs(self.ANALYSIS_DIR, exist_ok=True)
        if not self.RUN_DIR.is_dir():
            print(f'WARNING: Cannot locate {self.RUN_DIR}, creating it...')
        ...
        self._drawFitCurve(ax, nArr, posArr, medians, self.percentErrorVsNFitPower, var='e')
        ...
        plt.tight_layout()
        plt.savefig(rf'{self.ANALYSIS_DIR}\percent-error-vs-n.png')
        plt.close()
    ...
    def plotTour(self, instanceName, runIdx=0):
        ...
        instanceFile = self.INSTANCE_DIR / f'{instanceName}.tsp'
        runFile = self.RUN_DIR / instanceName / f'{instanceName}_{runIdx}.txt'
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(rf'{self.ANALYSIS_DIR}\{instanceName}-tour-{runIdx}.png')
            plt.close()
    ...
    def plotConvergence(self, instanceName, runIdx=0):
        ...
        try:
            ...
            ax.grid(True, linestyle='--', linewidth=0.5, alpha=0.7)
            plt.tight_layout()
            plt.savefig(rf'{self.ANALYSIS_DIR}\{instanceName}-convergence-{runIdx}.png')
            plt.close()
    ...
    def plotAggConvergence(self):
        ...
        try:
            ...
            if not allCurves:
                print(f'No convergence data found in {self.RUN_DIR}')
                return
            ...
            if self.showAggConvergenceCurves:
            ...
            if self.showAggConvergenceStats:
            ...
            plt.tight_layout()
            plt.savefig(rf'{self.ANALYSIS_DIR}\convergence-aggregate.png')
            plt.close()
    ...
    def plotHeatMap(self, instanceName):
        ...
        instanceFile = self.INSTANCE_DIR / f'{instanceName}.tsp'
        runDir = self.RUN_DIR / instanceName
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(rf'{self.ANALYSIS_DIR}\{instanceName}_heatmap.png')
            plt.close()
