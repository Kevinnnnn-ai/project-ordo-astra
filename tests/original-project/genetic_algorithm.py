...
@numba.njit(parallel=CAN_PARALLEL, nogil=NO_GIL, cache=CAN_CACHE)
def _twoOptTopK(count, pop, fits, distMatrix, n, threshold):
...
def runGA(instance, runNum):
    RUN_DIR = ROOT_DIR / 'stdout' / 'runs' / instance.instanceName
    if not RUN_DIR.is_dir():
        print(f'WARNING: Cannot locate {RUN_DIR}, creating it...')
        os.makedirs(RUN_DIR, exist_ok=True)

    runFile = RUN_DIR / f'{instance.instanceName}_{runNum}.txt'
    log = Log(runFile)
    ...
    for gen in range(1, MAX_GENS + 1):
        ...
        threshold = np.percentile(newFitsRest, 100)
        newPopRest, newFitsRest = _twoOptTopK(POP_SIZE - 1, newPopRest, newFitsRest, distMatrix, n, threshold)
